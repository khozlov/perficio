...
var getUser = function(id, req, res) {
  User.findById(id).select('-email').exec(function(err, usr) {
    if (err || usr === null) {
      res.status(404).json({
        error: "User not found"
      });
    } else {
      usr.getAchievementsAndTemplates(req.isAuthenticated(), function(err, result) {
        if (err) {
          res.status(503).json({
...
module.exports.current = function(req, res) {
  getUser(req.session.passport.user, req, res);
}
                ...
                achievement = Achievement.findOne({
                  ...
                  if (err) {
                    ...
                    if (achievement !== null) {
                      ...
                      achievement.grantedBy.push(req.session.passport.user);
                      achievement.template = template.id;
                      achievement.name = template.name;
                      achievement.private = template.private;
                      achievement.tags = template.tags;
...
module.exports.index = function(req, res) {
  User.find().select('name photoUrl').exec(function(err, users) {
    var populateUserAchievements = function(user, callback) {
      user.getAchievements('-createdAt', req.isAuthenticated(), function(err, achievements) {
        if (err) {
          callback(err);
        } else {
